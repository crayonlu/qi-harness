...
// Statusline is optional / not default — powerline+emoji clashes with Pi-native chrome
assert.equal(harnessPkg.dependencies["@narumitw/pi-statusline"], undefined);
assert.ok(!harnessPkg.bundledDependencies?.includes("@narumitw/pi-statusline"));
assert.ok(!(harnessPkg.pi?.extensions ?? []).some((e) => String(e).includes("pi-statusline")));
